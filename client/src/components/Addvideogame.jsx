...
function Addvideogame() {
  ...
  function Validate(input) {
    ...
    if (input.rating && (input.rating < 1 || input.rating > 5))
      errors.rating = "Rating must be in range 1-5";

    if (input.rating) setActiveError({ ...activeError, rating: false });
    if (input.name) setActiveError({ ...activeError, name: false });
    if (input.description)
      setActiveError({ ...activeError, description: false });
    if (input.platforms?.length !== 0)
      setActiveError({ ...activeError, platforms: false });
      ...
      <div className="principal">
        <form onSubmit={onSubmit}>
          <div className="detname">
            ...
            ></input>
            {activeError.name && <label>{errors.name}</label>}
          </div>
          <br />
          <div className="detdescription">
            ...
            <textarea
              ...
              required
            />
            {activeError.description && <label>{errors.description}</label>}
          </div>
          <br />
          ...
          <div className="detrating">
            ...
            ></input>
            {activeError.rating && <label>{errors.rating}</label>}
          </div>
          <br />
          <div className="detplatforms">
            <label className="selectores">
              Platforms:
              <br />
              <br />
              <select
                ...
                required
              >
                <option key="a" value="Wii">WII</option>
                <option key="b" value="PS5">PS5</option>
                <option key="c" value="PS4">PS4</option>
                <option key="d" value="PS3">PS3</option>
                <option key="e" value="Xbox S/X">Xbox S/X</option>
                <option key="f" value="Xbox One">Xbox One</option>
                <option key="g" value="Xbox 360">Xbox 360</option>
                <option key="h" value="Nintendo">Nintendo</option>
                <option key="i" value="PC">PC</option>
              </select>
            </label>

            {activeError.platforms && <label>{errors.platforms}</label>}
          </div>
          <br />
          ...
          <div className="detsubmit">
            <input
              type={
                !errors.platforms && !errors.name && !errors.description
                  ? "submit"
                  : "button"
              }
              onClick={handleErrorClick}
              value="Submit videogame"
              className="butn"
            ></input>
          </div>
          <br />
          ...
          <div className="selectedThings">
            {videogame.platforms.map((el, i) => (
              <div>
                <p key={i + 500}>
                  {el} <button onClick={(e) => handleDeletePlat(el)}>X</button>
                </p>
              </div>
            ))}
          </div>
